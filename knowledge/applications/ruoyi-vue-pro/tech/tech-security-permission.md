...
适用于管理后台接口权限、App 端登录态判断、数据权限、角色权限和租户相关安全边界。

## 不适用范围

不覆盖目标项目具体角色及其最终权限矩阵；这些应进入 `rule/` 或对应 Change。
...
- 谨慎把后台权限模型直接套到 App 端。
- 谨慎复用上游角色、租户和菜单配置，必须先匹配目标项目的组织与权限模型。
...
| 问题 | 当前状态 | 影响 |
| --- | --- | --- |
| 目标项目角色模型是否沿用上游 RBAC | 待初始化 | 影响菜单权限、按钮权限和数据权限 |
| 目标项目如何定义部门与数据权限 | 待初始化 | 影响数据范围实现 |
| App 端是否单独设计权限体系 | 待初始化 | 影响 app API 登录与鉴权方式 |
