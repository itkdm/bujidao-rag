---
title: "菜单路由"
source_url: "https://doc.iocoder.cn/vue3/route/"
official_path: "/01.开发指南/30.前端手册 Vue 3.x/02.菜单路由.html"
crawled_at: "2026-08-09T06:54:52.440Z"
---
...
默认情况下，外链会在【新窗口】打开。如果希望在后台管理系统内部通过 iframe 打开，可以在外链地址后拼接 `?_iframe` 参数，例如：
...
- 动态路由在生产环境下会默认使用路由懒加载，实现方式参考 [import.meta.glob('../views/**/*.{vue,tsx}')](https://github.com/yudaocode/yudao-ui-admin-vue3/blob/master/src/utils/routerHelper.ts#L6) 方法的判断
...
// 推荐：使用对象形式跳转，便于同时传递 query、params、hash
push({
...
// 动态路由参数，例如路由 path 为 /system/user/:id
push({
...
动态菜单支持在【路由地址】或【组件路径】中通过 `?`、`#` 携带页面参数，用于对齐 Vue Router 的 `query`、`params`、`hash` 能力。

常见写法如下：

```
/system/user?deptId=100
/bpm/process-instance/detail?id=100#task
```

对应页面中，可以通过 `useRoute()` 获取参数：
...
```
/system/user/:id?id=100&amp;tab=role
```

系统会自动将 `id=100` 提取为 `params.id`，并将剩余的 `tab=role` 保留为 `query.tab`。跳转时最终会生成类似 `/system/user/100?tab=role` 的地址。

友情提示

动态参数名来自路由地址中的 `:参数名`，例如 `:id`、`:userId`。如果查询参数里没有对应值，则不会替换该动态参数。
...
注意，普通外链默认会在【新窗口】打开。如果希望在系统内部通过 iframe 打开，可以在外链地址后拼接 `?_iframe` 参数，例如 `https://www.iocoder.cn?_iframe`。

### 3.2 新增菜单

![新增菜单](https://doc.iocoder.cn/img/Vue3/%E8%8F%9C%E5%8D%95%E8%B7%AF%E7%94%B1/07.png)

“路由地址”支持通过 `?` 拼接查询参数、通过 `#` 拼接 hash，也支持动态参数。例如：

```
/bpm/process-instance/detail?id=100#task
/system/user/:id?id=100
```

页面中可通过 `useRoute()` 读取 `query`、`params`、`hash`。
