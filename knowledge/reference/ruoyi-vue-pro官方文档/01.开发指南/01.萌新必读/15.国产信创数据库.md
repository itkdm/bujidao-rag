---
title: "国产信创数据库（DM 达梦、大金、OpenGauss、瀚高）"
source_url: "https://doc.iocoder.cn/xinchuang-db/"
official_path: "/01.开发指南/01.萌新必读/15.国产信创数据库.html"
crawled_at: "2026-08-09T06:54:49.586Z"
---
## 1. DM 达梦数据库
...
- [x86_64 版本](https://kingbase.oss-cn-beijing.aliyuncs.com/KESV8R3/V009R001C001B0025-%E5%AE%89%E8%A3%85%E5%8C%85-docker/x86_64/kdb_x86_64_V009R001C001B0025.tar) 【Windows 选择这个】
- [aarch64 版本](https://kingbase.oss-cn-beijing.aliyuncs.com/KESV8R3/V009R001C001B0025-%E5%AE%89%E8%A3%85%E5%8C%85-docker/aarch64/kdb_aarch64_V009R001C001B0025.tar) 【MacBook Apple Silicon 选择这个】
...
- [《球友基于 AI + 文档适配成功》](https://t.zsxq.com/hMzBJ)
...
① 参考 [《用 JetBrains DataGrip 连接 Huawei openGauss》](https://blog.itdevwu.com/post/1632/) 文档，连接 openGauss 数据库。
...
注意，目前项目按照 PostgreSQL 兼容模式适配瀚高数据库。
...
① 使用 DataGrip、DBeaver 等数据库工具，选择 PostgreSQL 数据库类型，连接瀚高数据库。

② 先执行项目中的 `sql/highgo/quartz.sql` 文件，再执行 `sql/highgo/ruoyi-vue-pro.sql` 文件。

友情提示

瀚高数据库兼容 PostgreSQL，所以项目 SQL、Quartz、Flowable 都按照 PostgreSQL 兼容模式处理。

### 4.3 如何启动项目？

① 项目已经在 `yudao-server/pom.xml` 引入 PostgreSQL JDBC 驱动。若你裁剪过依赖，请确认最终运行包包含 `org.postgresql:postgresql` 依赖。例如说：
...
② 修改 `application-local.yaml` 配置文件，将数据库的 `url`、`username`、`password` 修改为瀚高数据库。例如说：
...
spring:
  datasource:
    dynamic:
      ...
      datasource:
        master:
          url: jdbc:postgresql://127.0.0.1:5866/ruoyi-vue-pro
          driver-class-name: org.postgresql.Driver
          username: root
          password: 123456
```

注意，如果本地配置里 `validation-query` 仍然是 `SELECT 1 FROM DUAL`，需要改成 `SELECT 1`，否则 PostgreSQL/瀚高会因为不存在 `DUAL` 表导致连接校验失败。

疑问：为什么 jdbc 使用 postgres 数据库连接？

因为瀚高数据库兼容 PostgreSQL，所以 JDBC 驱动使用的是 `org.postgresql.Driver`。这样，后续和 Quartz 定时任务、Flowable 工作流等等，都是可以兼容的。
