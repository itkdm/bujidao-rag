...
Change 从 `proposed/` 移动到 `implemented/` 前，先完成一次完整预检：运行知识库校验和 `git diff --check`，并确认 `change.md` 已将“方案/验收标准/风险与约束”转换为与当前实现一致的“最终决策/验证结果/影响与后果”，正文不再把当前状态写成将来时或引用失效的 `proposed/` 当前路径。移动目录后必须再次执行完整终检；预检或终检任一失败，都不得提交该生命周期移动。
